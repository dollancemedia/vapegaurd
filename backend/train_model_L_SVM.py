...
def train_and_save_model(limit=None, save=True, skip_predict=False,
                         C=1.0, max_iter=10000, use_scaler=True, calibrate_cv=3,
                         model_filename=None):
    """
    ...
    # Build LinearSVC pipeline and wrap with CalibratedClassifierCV to get probabilities
    steps = []
    if use_scaler:
        steps.append(("scaler", StandardScaler()))
    steps.append(("svc", LinearSVC(C=float(C), max_iter=int(max_iter))))
    base_clf = Pipeline(steps)
    clf = CalibratedClassifierCV(base_clf, cv=int(calibrate_cv))
    clf.fit(X, y)

    # Persist model to backend/models (inference.py expects this path) unless save=False
    # allow custom filename (useful for CI/testing); default to models/linear_svc_model.joblib
    if model_filename is None:
        out_path = os.path.join(os.path.dirname(__file__), "models", "linear_svc_model.joblib")
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Train LinearSVC on historical events; hold out newest for prediction.")
    ...
    # LinearSVC options
    parser.add_argument("--C", type=float, default=1.0, help="Regularization parameter C for LinearSVC")
    parser.add_argument("--max-iter", type=int, default=10000, help="Max iterations for LinearSVC")
    parser.add_argument("--calibrate-cv", type=int, default=3, help="Number of CV folds for probability calibration")
    parser.add_argument("--no-scale", action="store_true", help="Disable StandardScaler in the pipeline")
    parser.add_argument("--model-file", type=str, default=None, help="Optional output model file path (default: models/linear_svc_model.joblib)")
