#--------IMPORTANT-------
# YOU MAY NEED TO CHANGE LINE 227 TO "xgb" IF THE DATA SAVE FOR "knn" ISN'T SETUP YET
...
def train_and_save_model(limit=None, save=True, skip_predict=False,
                         n_neighbors=5, weights="uniform", use_scaler=True,
                         model_filename=None):
    """
    ...
    # Build classifier pipeline according to CLI options
    steps = []
    if use_scaler:
        steps.append(("scaler", StandardScaler()))
    steps.append(("knn", KNeighborsClassifier(n_neighbors=int(n_neighbors), weights=weights)))
    clf = Pipeline(steps)
    clf.fit(X, y)

    # Persist model to backend/models (inference.py may expect the xgboost path; use knn path for this script)
    # allow custom filename (useful for CI/testing); default to models/knn_model.joblib
    if model_filename is None:
        out_path = os.path.join(os.path.dirname(__file__), "models", "knn_model.joblib") #-----------------------if errors, change "knn" to "xgb"
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Train KNN on historical events; hold out newest for prediction.")
    ...
    # KNN hyperparameters
    parser.add_argument("--n-neighbors", type=int, default=5, help="Number of neighbors for KNN (default: 5)")
    parser.add_argument("--weights", choices=["uniform", "distance"], default="uniform", help="Weight function used in prediction")
    parser.add_argument("--no-scale", action="store_true", help="Disable StandardScaler in the pipeline")
    parser.add_argument("--model-file", type=str, default=None, help="Optional output model file path (default: models/knn_model.joblib)")
