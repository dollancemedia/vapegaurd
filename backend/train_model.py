...
def train_and_save_model(limit=None, save=True, skip_predict=False):
    ...
    out_path = os.path.join(os.path.dirname(__file__), "models", "xgb_model.joblib")
    if save:
...
def main():
    parser = argparse.ArgumentParser(description="Train XGBoost on historical events; hold out newest for prediction.")
    ...
    train_and_save_model(limit=args.limit, save=not args.no_save, skip_predict=args.skip_predict)
