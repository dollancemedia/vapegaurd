...
def train_and_save_model(limit=None, save=True, skip_predict=False,
                         C=1.0, kernel="rbf", gamma="scale", degree=3,
                         use_scaler=True, model_filename=None):
    """
    ...
    # Build SVC pipeline (enable probability) with user-configurable hyperparams
    steps = []
    if use_scaler:
        steps.append(("scaler", StandardScaler()))
    steps.append(("svc", SVC(C=float(C), kernel=kernel, gamma=gamma, degree=int(degree), probability=True)))
    clf = Pipeline(steps)
    clf.fit(X, y)

    # Persist model to backend/models (inference.py expects this path) unless save=False
    # allow custom filename; default to models/svc_model.joblib
    if model_filename is None:
        out_path = os.path.join(os.path.dirname(__file__), "models", "svc_model.joblib")
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Train SVC on historical events; hold out newest for prediction.")
    ...
    # SVC hyperparameters
    parser.add_argument("--C", type=float, default=1.0, help="Regularization parameter C for SVC")
    parser.add_argument("--kernel", choices=["linear", "poly", "rbf", "sigmoid"], default="rbf", help="Kernel type for SVC")
    parser.add_argument("--gamma", default="scale", help="Kernel coefficient for 'rbf', 'poly' and 'sigmoid' (auto/scale or float)")
    ...
    parser.add_argument("--model-file", type=str, default=None, help="Optional output model file path (default: models/svc_model.joblib)")
